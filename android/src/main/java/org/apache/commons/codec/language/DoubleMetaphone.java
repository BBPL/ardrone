...
public class DoubleMetaphone implements StringEncoder {
    ...
    private static final String[] L_R_N_M_B_H_F_V_W_SPACE = new String[]{"L", "R", "N", "M", "B", "H", "F", GPSTagConstants.GPS_TAG_GPS_STATUS_VALUE_MEASUREMENT_INTEROPERABILITY, "W", " "};
    ...
    private boolean conditionCH1(String str, int i) {
        if (!(contains(str, 0, 4, "VAN ", "VON ") || contains(str, 0, 3, "SCH"))) {
            if (!(contains(str, i - 2, 6, "ORCHES", "ARCHIT", "ORCHID") || contains(str, i + 2, 1, "T", "S"))) {
                if (!((contains(str, i - 1, 1, GPSTagConstants.GPS_TAG_GPS_STATUS_VALUE_MEASUREMENT_IN_PROGRESS, "O", "U", "E") || i == 0) && (contains(str, i + 2, 1, L_R_N_M_B_H_F_V_W_SPACE) || i + 1 == str.length() - 1))) {
                    return false;
                }
            }
        }
    ...
    private boolean conditionL0(String str, int i) {
        ...
        return (contains(str, str.length() + -2, 2, "AS", "OS") || contains(str, str.length() - 1, 1, GPSTagConstants.GPS_TAG_GPS_STATUS_VALUE_MEASUREMENT_IN_PROGRESS, "O")) && contains(str, i - 1, 4, "ALLE");
    }

    private boolean conditionM0(String str, int i) {
        return charAt(str, i + 1) == 'M' || (contains(str, i - 1, 3, "UMB") && (i + 1 == str.length() - 1 || contains(str, i + 2, 2, "ER")));
    }
    ...
    private int handleC(String str, DoubleMetaphoneResult doubleMetaphoneResult, int i) {
        if (conditionC0(str, i)) {
            ...
            if (contains(str, i, 2, "CZ") && !contains(str, i - 2, 4, "WICZ")) {
                ...
                return (!contains(str, i + 1, 1, "C", "K", "Q") || contains(str, i + 1, 2, "CE", "CI")) ? i + 1 : i + 2;
            }
        }
    }

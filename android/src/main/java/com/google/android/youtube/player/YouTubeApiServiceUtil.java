package com.google.android.youtube.player;

import android.content.Context;
import android.content.pm.PackageInfo;
import android.content.pm.PackageManager;
import android.content.pm.PackageManager.NameNotFoundException;
import android.util.Base64;
import com.google.android.youtube.player.internal.C0444z;
import java.io.ByteArrayInputStream;
import java.security.cert.CertificateException;
import java.security.cert.CertificateFactory;
import java.security.cert.X509Certificate;
import java.util.Arrays;

public final class YouTubeApiServiceUtil {
    private static final byte[][] f132a = new byte[][]{Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0), Base64.decode("", 0)};

    private YouTubeApiServiceUtil() {
    }

    private static boolean m1407a(PackageInfo packageInfo) {
        try {
            ((X509Certificate) CertificateFactory.getInstance("X509").generateCertificate(new ByteArrayInputStream(packageInfo.signatures[0].toByteArray()))).checkValidity();
            if (packageInfo.signatures.length != 1) {
                return false;
            }
            byte[] toByteArray = packageInfo.signatures[0].toByteArray();
            for (byte[] equals : f132a) {
                if (Arrays.equals(equals, toByteArray)) {
                    return true;
                }
            }
            return false;
        } catch (CertificateException e) {
            return false;
        }
    }

    public static YouTubeInitializationResult isYouTubeApiServiceAvailable(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            String a = C0444z.m1733a(context);
            return !m1407a(packageManager.getPackageInfo(a, 64)) ? YouTubeInitializationResult.SERVICE_INVALID : C0444z.m1734a(context, a) ? YouTubeInitializationResult.SERVICE_VERSION_UPDATE_REQUIRED : !packageManager.getApplicationInfo(a, 0).enabled ? YouTubeInitializationResult.SERVICE_DISABLED : YouTubeInitializationResult.SUCCESS;
        } catch (NameNotFoundException e) {
            return YouTubeInitializationResult.SERVICE_MISSING;
        }
    }
}
